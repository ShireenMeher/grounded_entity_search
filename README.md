...
**Incident: a too-aggressive timeout caused a real production outage.** After the sequential-call fix, `search_timeout_seconds` was tuned down to 3s against local network conditions to shave latency. In production on Render, an occasional slow SerpAPI response caused *all three* sequential variant calls to time out together, so `search_multi` returned zero results — the pipeline then had nothing to scrape and surfaced "no entities found" for a perfectly valid query. The fix has two parts: the timeout was loosened back to 6s for more margin per call, and `search_multi` now retries the original query once with a 3x longer timeout if every variant comes back empty, rather than giving up. The retry only adds latency to the already-failing case, never the common path. Lesson: a timeout tuned against one network environment isn't safe to assume for another — the fix that gets you from 30s to 15s isn't free if it turns "slow" into "zero results" under different conditions.
